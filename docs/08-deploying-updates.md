...
it. It ends by printing one command. SSH to the box (`ssh petaronline@organic.petaronline.us`,
then become root) and paste it:

```bash
/opt/vass-organic/install-patch.sh /home/petaronline/public_html/vass/vass-organic-patch-<stamp>.zip && \
  ( sleep 5; ss -tln | grep -q ':4041' || ( cd /opt/vass-organic && docker compose restart backend ) )
```

Verify:

```bash
curl -s -o /dev/null -w '%{http_code}\n' https://organic.petaronline.us/api/health   # 200
```
...
3. **Clean staging before install.** `install-patch.sh` does `unzip -o` (never
   deletes) then `cp -rf` into `/opt/vass-organic`. Stale files in
   `/home/petaronline/public_html/vass/{backend,frontend}` get re-injected and
   can break the rebuild. `deploy.sh` removes them first.
4. **Watch the port-4041 race.** With `network_mode: host`, a force-recreate can
   briefly collide on 4041 and kill the new API. The install command above
   restarts the backend if 4041 isn't listening after 5s.
