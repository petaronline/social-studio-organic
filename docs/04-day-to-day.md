...
All commands assume you're SSH'd into the server and in `/opt/vass-organic`.
...
```bash
docker compose exec postgres psql -U vassorganic -d vassorganic
```
...
```bash
docker compose exec postgres psql -U vassorganic -d vassorganic -c \
...
```bash
mkdir -p /opt/vass-organic/backups
docker compose exec -T postgres pg_dump -U vassorganic vassorganic | gzip > \
  /opt/vass-organic/backups/vass-organic-$(date +%Y%m%d-%H%M%S).sql.gz
```

Output file: `/opt/vass-organic/backups/vass-organic-20260522-143022.sql.gz`
...
```bash
cat > /opt/vass-organic/scripts/backup.sh << 'EOF'
#!/bin/bash
# Nightly backup — keeps 30 days of dumps
set -e

BACKUP_DIR="/opt/vass-organic/backups"
mkdir -p "$BACKUP_DIR"
...
docker compose exec -T postgres pg_dump -U vassorganic vassorganic | gzip > "$FILENAME"
...
chmod +x /opt/vass-organic/scripts/backup.sh
```
...
```
0 3 * * * /opt/vass-organic/scripts/backup.sh >> /var/log/vass-organic-backup.log 2>&1
```
...
# Restore
gunzip < /opt/vass-organic/backups/vass-organic-20260522-143022.sql.gz | \
  docker compose exec -T postgres psql -U vassorganic vassorganic
