# Vass Organic — TikTok Publishing Handoff (resume in new chat)

Last updated: 2026-05-29. Hand this to a new chat to continue.

## Project basics
- Vass Organic = social publishing (IG, FB Pages, Threads, TikTok, LinkedIn), split out of the Vass ads app. Live at https://organic.petaronline.us, code at /opt/vass.
- Stack: Node 20 + TS + Express + Postgres 16 + Redis + BullMQ (backend port 4041); Next.js 15 App Router + Tailwind + Lucide (port 3031). Docker compose, backend container = `vass-organic-backend` (app root `/app`). Apache reverse proxy.
- Install a patch: `/opt/vass-organic/install-patch.sh /home/petaronline/public_html/vass-organic/vass-organic-patch-X.X.X.zip` (self-updates, runs migrations, restarts backend).
- Diagnostics run from inside the container: `docker cp <file> vass-organic-backend:/app/ && docker exec -it vass-organic-backend sh -c 'cd /app && node <file>'`. Watch logs: `docker logs -f vass-organic-backend 2>&1 | grep -iE "tiktok|publish|error"`.
...
The publisher uses PULL_FROM_URL — it hands TikTok a Vass media URL (`https://organic.petaronline.us/...`) to fetch the video from. TikTok refuses to pull from any domain not verified as a trusted URL prefix in the developer portal.
...
- Add + verify `https://organic.petaronline.us/` via the method TikTok offers (DNS TXT record OR a verification file at the domain root).
