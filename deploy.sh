#!/usr/bin/env bash
#
# Vass Organic deploy helper — run from your Mac, in the repo root:  ./deploy.sh
#
...
#
# ---- Differences from the ads app's deploy.sh ----
# Separate domain, separate staging dir, separate /opt tree, separate port.
# This script can never touch the ads install; if you find yourself editing the
# paths below to point at /opt/vass, stop — that is the ads app's deploy.
#
set -euo pipefail

# --- config ---
SERVER="petaronline@organic.petaronline.us"
STAGING="/home/petaronline/public_html/vass-organic"
...
ZIP="vass-organic-patch-${STAMP}.zip"
ZIP_LOCAL="/tmp/${ZIP}"
...
# UI changes actually go live.
echo "    rm -rf $STAGING/backend $STAGING/frontend && \\"
echo "      $INSTALL_ROOT/install-patch.sh $STAGING/$ZIP && \\"
echo "      cd $INSTALL_ROOT && docker compose build --no-cache frontend && \\"
echo "      docker compose up -d --force-recreate frontend && \\"
echo "      ( sleep 5; ss -tln | grep -q ':$BACKEND_PORT' || docker compose restart backend )"
echo
